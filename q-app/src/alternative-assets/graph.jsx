...
export function RelationshipGraph({ theme }) {
  const ref = useRef(null)
  const inView = useInView(ref, { threshold: 0.3 })
  const { relationshipNodes: nodes, relationshipEdges: edges } = ALT_DATA
        ...
        {edges.map(([aId, bId], i) => {
          ...
          return (
            <path key={i} d={d}
              ...
              style={{ transition: `stroke-dashoffset 1.2s ${edgeDelay(i)}s cubic-bezier(0.4, 0, 0.2, 1)` }}
            />
          )
        })}
...
const SUB_FS = 9
const SUB_LINE_H = 12
const LABEL_SUB_GAP = 6

const LABEL_CHAR_W = LABEL_FS * 0.58
const SUB_CHAR_W = SUB_FS * 0.62 + 0.5
...
function NodeShape({ x, y, node, theme }) {
  ...
  const labelY = stackTop + LABEL_FS - 2
  const subStartY = stackTop + LABEL_FS + LABEL_SUB_GAP + SUB_FS - 1
...
export function LifecycleFlow({ theme }) {
  const ref = useRef(null)
  const inView = useInView(ref)
  const stages = ALT_DATA.lifecycle
  return (
    <div ref={ref} style={{
