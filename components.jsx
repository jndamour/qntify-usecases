// Reusable UI components: nav bar, marker, crosshair reticle, detail panel, tweaks panel

const { useState, useEffect, useRef, useMemo, useCallback } = React;

// ─── Nav bar (matches qntify.net look) ───────────────────────────────────
...
// ─── Crosshair reticle ─────────────────────────────────────────────────────
function Reticle({ cx, cy, theme, size = 80 }) {
  const half = size / 2;
  const tick = 8;
  const gap = 4;
  return (
...
function Marker({ x, y, active, status, theme, onClick, index }) {
...
Object.assign(window, { NavBar, LiveClock, Reticle, Marker });
